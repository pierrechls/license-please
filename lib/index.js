...
module.exports = function (list, force, license, year, name) {
    ...
    if (list) {
      output.help('List of possible licenses :', true, true)
      ...
      fs.access(LICENSE_FILE_NAME, fs.constants.F_OK, (err) => {
        ...
        if (!err && !force) {
          ...
          fs.writeFile(LICENSE_FILE_NAME, content, function(err) {
            err ? output.error(err, true, true) : output.success(`New LICENSE file generated with the '${license}' license`, true, true)
          })
        }
      })

    } else {
      output.error(`The '${license}' license doesn't seem to exist`, true, true)
    }
}
